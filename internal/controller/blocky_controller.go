...
const blockyFinalizer = "apps.blocky.unmango.net/finalizer"

// Definitions to manage status conditions
const (
	// typeAvailableBlocky represents the status of the Deployment reconciliation
	typeAvailableBlocky = "Available"
	// typeDegradedBlocky represents the status used when the custom resource is deleted and the finalizer operations are must to occur.
	typeDegradedBlocky = "Degraded"
)

// BlockyReconciler reconciles a Blocky object
type BlockyReconciler struct {
	client.Client
...
//+kubebuilder:rbac:groups=apps.blocky.unmango.net,resources=blockies,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=apps.blocky.unmango.net,resources=blockies/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=apps.blocky.unmango.net,resources=blockies/finalizers,verbs=update
...
func (r *BlockyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if blocky.Status.Conditions == nil || len(blocky.Status.Conditions) == 0 {
		...
		// Let's re-fetch the blocky Custom Resource after update the status
	...
	// Check if the Blocky instance is marked to be deleted, which is
	...
	if isBlockyMarkedToBeDeleted {
		if controllerutil.ContainsFinalizer(blocky, blockyFinalizer) {
			...
			r.doFinalizerOperationsForBlocky(blocky)

			// TODO(user): If you add operations to the doFinalizerOperationsForBlocky method
	...
	err = r.Get(ctx, types.NamespacedName{Name: blocky.Name, Namespace: blocky.Namespace}, found)
	...
	// The CRD API is defining that the Blocky type, have a BlockySpec.Size field
	// to set the quantity of Deployment instances is the desired state on the cluster.
	// Therefore, the following code will ensure the Deployment size is the same as defined
	// via the Size spec of the Custom Resource which we are reconciling.
	size := blocky.Spec.Size
...
// finalizeBlocky will perform the required operations before delete the CR.
func (r *BlockyReconciler) doFinalizerOperationsForBlocky(cr *appsv1alpha1.Blocky) {
...
// deploymentForBlocky returns a Blocky Deployment object
func (r *BlockyReconciler) deploymentForBlocky(
	blocky *appsv1alpha1.Blocky) (*appsv1.Deployment, error) {
	ls := labelsForBlocky(blocky.Name)
	replicas := blocky.Spec.Size

	// Get the Operand image
	image, err := imageForBlocky()
	if err != nil {
		return nil, err
	}
				...
				Spec: corev1.PodSpec{
					...
					Containers: []corev1.Container{{
						...
						Ports: []corev1.ContainerPort{{
							ContainerPort: blocky.Spec.ContainerPort,
							Name:          "blocky",
						}},
					}},
				},
	...
	if err := ctrl.SetControllerReference(blocky, dep, r.Scheme); err != nil {
		return nil, err
	}
	return dep, nil
}

// labelsForBlocky returns the labels for selecting the resources
// More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/common-labels/
func labelsForBlocky(name string) map[string]string {
	var imageTag string
	image, err := imageForBlocky()
	if err == nil {
		imageTag = strings.Split(image, ":")[1]
	}
	return map[string]string{"app.kubernetes.io/name": "Blocky",
		"app.kubernetes.io/instance":   name,
		"app.kubernetes.io/version":    imageTag,
		"app.kubernetes.io/part-of":    "blocky-controller",
		"app.kubernetes.io/created-by": "controller-manager",
	}
}

// imageForBlocky gets the Operand image which is managed by this controller
// from the BLOCKY_IMAGE environment variable defined in the config/manager/manager.yaml
func imageForBlocky() (string, error) {
	var imageEnvVar = "BLOCKY_IMAGE"
...
func (r *BlockyReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&appsv1alpha1.Blocky{}).
